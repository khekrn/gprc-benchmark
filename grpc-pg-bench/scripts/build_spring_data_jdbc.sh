#!/usr/bin/env bash
# Build the Spring Boot gRPC + virtual-threads + Spring Data JDBC server.
#
# Spring Boot produces a single executable fat jar; it is copied to
# bin/spring-data-jdbc-bench.jar and runs via:
#   java $JVM_OPTS -jar bin/spring-data-jdbc-bench.jar
#
...
MOD_DIR="${ROOT_DIR}/spring-data-jdbc"
...
echo ">> Building spring-data-jdbc (mvn clean package)"
( cd "${MOD_DIR}" && mvn -B -q clean package -DskipTests )

JAR="${MOD_DIR}/target/spring-data-jdbc-bench-1.0.0.jar"
if [ -f "${JAR}" ]; then
  mkdir -p "${ROOT_DIR}/bin"
  cp "${JAR}" "${ROOT_DIR}/bin/spring-data-jdbc-bench.jar"
  echo ">> Done. App at ${ROOT_DIR}/bin/spring-data-jdbc-bench.jar"
else
